...
function Header()
	{
		...
		$tahun1=$ci->session->tahun1;
        $tahun2=$ci->session->tahun2;
        if($tahun1=='')
        {
            $tahun1=date('Y')-4;
        }
		...
		$this->Cell(185, 5, 'REKAP PUBLIKASI SDM', '', 0,'C');	
		if($unit!='')
		{
		$this->Ln();
		...
		$this->Ln();
		$this->Cell(185, 5, 'Tahun '.$tahun, '', 0,'C');
		$this->Ln(7);
		$kolom=$tahun2-$tahun1+1;
		$lebar=15*$kolom;
		$this->SetFont("Arial", "B", 10);
		$this->Cell(10, 12, 'No', 'LTBR', 0,'C');
		$this->Cell(70, 12, 'Media Publikasi', 'TBR', 0,'C');	
		$this->Cell($lebar, 7, 'Tahun', 'TR', 0,'C');
		$this->Cell(20, 12, 'Jumlah', 'LBTR', 0,'C');
