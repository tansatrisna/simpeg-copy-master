...
$ttl=0;
          ...
          foreach ($record as $row) {
            $no++;
            $dl=jlhJkAgama($row['kderef'],'DOSEN','L',$unit);
            $dp=jlhJkAgama($row['kderef'],'DOSEN','P',$unit);
            $pl=jlhJkAgama($row['kderef'],'PEGAWAI','L',$unit);
            $pp=jlhJkAgama($row['kderef'],'PEGAWAI','P',$unit);
            $tl=$dl+$pl;
            $tp=$dp+$pp;
            $ts=$tl+$tp;
