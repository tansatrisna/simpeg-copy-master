    ...
    <div class="row">
    	<div class="col-md-6">
    		<div class="panel panel-inverse">
                <div class="panel-heading">
                    <div class="panel-heading-btn">
                        <?=aksiDetail('statistik/detail','pendidikan','Lihat Statistik Lengkap')?>
                    </div>
                      ...
                      <tbody>
                           ...
                           $tpr=0;
                            foreach ($record as $row) {
                              $no++;
                              $lk=viewJkPend('L',$row['kode_pendidikan'],$jenis,$unit);
                              $pr=viewJkPend('P',$row['kode_pendidikan'],$jenis,$unit);
                              echo '<tr>
                                    <td>'.$no.'</td>
                                    <td>'.viewKodeApp('JENJPEND',$row['kode_pendidikan']).'</td>
                                    <td>'.$lk.'</td>
                                    <td>'.$pr.'</td>
                                    <td>'.$row['jlh'].'</td>
                                  </tr>';
                              array_push($jk, ["label"=>viewKodeApp('JENJPEND',$row['kode_pendidikan']), "jlh"=>$row['jlh']]);
                              $tlk=$tlk+$lk;
                              $tpr=$tpr+$pr;
                              $total=$total+$row['jlh'];
                            }
