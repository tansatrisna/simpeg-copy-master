...
class Model_utama extends CI_model{
   /* public function kategori($id){
    ...
    public function view_group_ordering($table,$group,$order,$ordering){
        $this->db->group_by($group);
        $this->db->order_by($order,$ordering);
        return $this->db->get($table);
    }

    public function view_where_group($table,$data,$group){
        $this->db->where($data);
        $this->db->group_by($group);
        return $this->db->get($table);
    }
    ...
    public function view_joinn_where($table1,$table2,$table3,$field,$field1,$data,$order,$ordering,$baris,$dari){
        $this->db->select('*');
        $this->db->from($table1);
        $this->db->join($table2, $table1.'.'.$field.'='.$table2.'.'.$field);
        $this->db->join($table3, $table1.'.'.$field1.'='.$table3.'.'.$field1);
